...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    """
    # TODO Your code here
    q_value_current = (theta @ current_state_vector)[tuple2index(action_index, object_index)]
    q_value_next = (theta @ next_state_vector)[tuple2index(action_index, object_index)]
    if not terminal:
        theta += ALPHA * ( reward + GAMMA * np.max(q_value_next)
                           - q_value_current) * current_state_vector
    else: theta += ALPHA * ( reward - q_value_current) * current_state_vector# TODO Your update here
# pragma: coderesponse end
