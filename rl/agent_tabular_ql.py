...
ALPHA = 1#e-6 #0.1  # learning rate for training
...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if np.random.rand() < epsilon:
        ...
        action_index, object_index = np.unravel_index(np.argmax(q_func[state_1, state_2, :, :], axis=None),q_func[state_1, state_2, :, :].shape)
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        (next_room_desc, next_quest_desc, reward, terminal) = \
            framework.step_game(current_room_desc, current_quest_desc, action_index,
                                object_index)
        ...
        if for_training:
            # update Q-function.
            # TODO Your code here
            tabular_q_learning(q_func, state_1, state_2, action_index, object_index, reward,
